...
Конечно! Вот оформленное описание **Этапа 3: Построение графа зависимостей** в том же стиле и формате, что и предыдущие этапы:
...
### Цель
Реализовать алгоритм построения графа зависимостей с использованием поиска в ширину (BFS) и добавить поддержку тестового режима для проверки логики.

### Требования
1. Построить граф зависимостей на основе прямых зависимостей, полученных на Этапе 2, с использованием BFS.
2. Учитывать максимальную глубину (`max_depth`) из конфигурации.
3. Поддерживать **тестовый режим**, в котором зависимости генерируются искусственно (без загрузки репозиториев).
4. Вывести граф зависимостей в текстовом формате: список рёбер вида `родитель -> зависимость = версия`.
5. Результат сохранён в Git.
...
<img width="1212" height="752" alt="image" src="https://github.com/user-attachments/assets/9c0dd63f-25a5-4fa7-9200-d701ccf0961f" />
...
Dependency Graph:
serde_json -> dtoa = "0.4"
serde_json -> num-traits = "0.2"
serde_json -> serde = "1.0"
```

> Вывод ограничен `max_depth = 1` в данном примере, так как на Этапе 3 реализован только **один уровень BFS** (прямые зависимости). В последующих версиях будет поддержка рекурсивного обхода до указанной глубины.

### Тестирование

- **Успешный случай (реальный режим)**: Используй репозиторий `serde-json` с версией `1.0.99`.  
  **<img width="1212" height="752" alt="image" src="https://github.com/user-attachments/assets/9c0dd63f-25a5-4fa7-9200-d701ccf0961f" />**  

- **Успешный случай (тестовый режим)**: `"test_mode": true`. Система использует искусственный граф, например:  
  ```json
  { "package_name": "X", "test_mode": true, "max_depth": 2 }
  ```  
  **<img width="1038" height="815" alt="image" src="https://github.com/user-attachments/assets/bfb56357-aad4-4ab8-9588-dfc5b3fc770d" />
**  
...
- **Ошибка: Отсутствие тега версии**  
  ...
  { "package_version": "999.0.0", ... }
  ```  
  **<img width="1230" height="603" alt="image" src="https://github.com/user-attachments/assets/199cb63b-bed6-464c-8bee-5facb2f02099" />
**  



> Примечание: в тестовом режиме (`test_mode: true`) проверяется только структура графа, загрузка репозиториев и парсинг `Cargo.toml` пропускаются.

### Реализация
- Добавлен параметр `test_mode` в схему валидации конфигурации.
- При `test_mode: false` используется логика Этапа 2 для получения зависимостей.
- При `test_mode: true` зависимости берутся из встроенного mock-графа (например, `{"A": ["B=1.0", "C=2.0"], "B": ["D=0.5"]}`).
